...
/**
 * Webkul MpsellersliderTwo MpsliderTwo controller
 */
...
class Deleteimage extends Action
{
        ...
        try {
            ...
            if ($collection->getSize() > 0) {
                foreach ($collection as $value) {
                    if ($this->helper->unserializeData($value->getSliderImgTwo())) {
                        $sliderImages = $this->helper->unserializeData($value->getSliderImgTwo());
                        $sliderImages = array_filter($sliderImages);
                        $key = array_search($deleteImages, array_column($sliderImages, 'image'));
                    } else {
                        $sliderImages = explode(',', $value->getSliderImgTwo());
                        $sliderImages = array_filter($sliderImages);
                    ...
                    $value->setSliderImgTwo($newSliderImags)->save();
            ...
            $this->helper->logDataInLogger(
                "Controller_MpsliderTwo_Deleteimage_execute Exception : ".$e->getMessage()
            );
            $this->getResponse()->representJson(
    ...
    private function deleteImage($path)
    {
        try {
            ...
            $this->helper->logDataInLogger(
                "Controller_MpsliderTwo_Deleteimage_deleteImage Exception : ".$e->getMessage()
            );
            return false;
        }
    }
