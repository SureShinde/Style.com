...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getSeller()
    {
        ...
        foreach ($model as $value) {
            ...
            $title_stylist = $value->getTitleStylist();
            $specialties_general = $value->getSpecialtiesGeneral();
            $specialties_work = $value->getSpecialtiesWork();
            $specialties_social = $value->getSpecialtiesSocial();
            $certification = $value->getCertification();
            $about_me = $value->getAboutMe();
            $zip_code_stylist = $value->getZipCodeStylist();
            $state_stylist = $value->getStateStylist();
            $street_stylist = $value->getStreetStylist();
            $city_stylist = $value->getCityStylist();
            ...
            if (strlen($title_stylist) <= 0) {
                $title_stylist = '';
            }
            if (strlen($specialties_general) <= 0) {
                $specialties_general = '';
            }
            if (strlen($specialties_work) <= 0) {
                $specialties_work = '';
            }
            if (strlen($specialties_social) <= 0) {
                $specialties_social = '';
            }
            ...
            if (strlen($zip_code_stylist) <= 0) {
                $zip_code_stylist = '';
            }
            if (strlen($state_stylist) <= 0) {
                $state_stylist = '';
            }
            if (strlen($street_stylist) <= 0) {
                $street_stylist = '';
            }
            if (strlen($city_stylist) <= 0) {
                $city_stylist = '';
            }
        ...
        $data['title_stylist'] = $title_stylist;
        $data['specialties_general'] = $specialties_general;
        $data['specialties_work'] = $specialties_work;
        $data['specialties_social'] = $specialties_social;
        $data['certification'] = $certification;
        $data['about_me'] = $about_me;
        $data['zip_code_stylist'] = $zip_code_stylist;
        $data['state_stylist'] = $state_stylist;
        $data['street_stylist'] = $street_stylist;
        $data['city_stylist'] = $city_stylist;
    ...
    public function getProfileUrl()
    {
        ...
        if ($targetUrl) {
            $temp = explode('/profile', $targetUrl);
            if (!isset($temp[1])) {
                $temp[1] = '';
            }
            $temp = explode('/', $temp[1]);
    ...
    public function getCollectionUrl()
    {
        $targetUrl = $this->getTargetUrlPath();
        if ($targetUrl) {
            $temp = explode('/collection', $targetUrl);
            if (!isset($temp[1])) {
                $temp[1] = '';
            }
            $temp = explode('/', $temp[1]);
    ...
    public function getLocationUrl()
    {
        $targetUrl = $this->getTargetUrlPath();
        if ($targetUrl) {
            $temp = explode('/location', $targetUrl);
            if (!isset($temp[1])) {
                $temp[1] = '';
            }
            $temp = explode('/', $temp[1]);
    ...
    public function getFeedbackUrl()
    {
        $targetUrl = $this->getTargetUrlPath();
        if ($targetUrl) {
            $temp = explode('/feedback', $targetUrl);
            if (!isset($temp[1])) {
                $temp[1] = '';
            }
            $temp = explode('/', $temp[1]);
    ...
    public function getProfileDetail($type = self::URL_TYPE_PROFILE)
    {
        ...
        if (!$shopUrl) {
            $targetUrl = $this->getTargetUrlPath();
            $shopUrl1 = explode("/", $targetUrl);
            $shopUrl = end($shopUrl1);
            // $shopUrl = $this->_request->getParam('id');
        }
        // var_dump($shopUrl);exit;
        if ($shopUrl) {
