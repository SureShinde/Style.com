...
/**
 *  Webkul Mpsellerslider Mpslider Saveslider controller
 */
class Savesliderimg extends Action
{
    ...
    public function execute()
    {
        ...
        if ($this->getRequest()->isPost() && $count == 0) {
            try {
                ...
                $this->helper->logDataInLogger("Controller_Mpslider_Savesliderimg execute : ".$e->getMessage());
    ...
    private function validateData($params)
    {
        ...
        try {
            ...
            $this->helper->logDataInLogger("Controller_Mpslider_Savesliderimg_validateData : ".$e->getMessage());
        }
        return [$data,$errors];
    }
    ...
    private function saveSliderImageSettings(
        ...
        if ($collection->getSize() > 0) {
            foreach ($collection as $value) {
                $sliderImage = $value->getSliderImg();
    ...
    private function saveSliderData($id, $sliderImages, $serial)
    {
        ...
        $sellerData->setSliderImg($sliderImages);
        $sellerData->setSliderSetting($serial);
        $sellerData->save();
    }
}
